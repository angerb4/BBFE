...
st.caption("Baseline HCV testing is preferably obtained within 48 hours. Baseline HBV testing is especially important when the exposed person is susceptible or immune status is uncertain.")
...
# Baseline/follow-up HBV testing for susceptible/uncertain exposed people
...
if susceptible_or_uncertain and source_hbv in ["Positive", "Unknown / pending"]:
    hbv_followup.insert(0, "Today: total anti-HBc baseline testing if not already collected")
...
if hbv_status in ["Unvaccinated or incompletely vaccinated", "Vaccination status unknown"] or (hbv_status == "Complete series; anti-HBs not previously documented" and anti_hbs_available == "Yes" and anti_hbs_value < 10):
    hbv_followup.append(f"Anti-HBs 1–2 months after final vaccine dose; use the later date if a range is needed, and delay until at least 6 months after HBIG if HBIG was given: {later_date(exposure_date, 60)} or later as clinically appropriate")
...
# HIV dated testing schedule
hiv_followup = []
if hiv_pep_indicated or source_hiv in ["Positive", "Unknown / pending"] or true_exposure:
    hiv_followup.append(f"Today: HIV Ag/Ab test; HIV-1 diagnostic NAT when indicated (especially recent injectable ARV/PrEP exposure or acute HIV concern)")
    ...
    if interim_hiv_needed:
        hiv_followup.append(f"Interim HIV testing: laboratory HIV Ag/Ab plus diagnostic HIV NAT on {later_date(exposure_date, 42)} (6 weeks; CDC range is 4–6 weeks, later date selected)")
...
if source_hcv in ["HCV RNA positive", "Anti-HCV positive; RNA unknown", "Unknown / cannot test"]:
    hcv_followup.append(f"Today/within 48 hours: exposed-person anti-HCV with reflex HCV RNA if positive")
...
if "Antacids or supplements containing aluminum, magnesium, calcium, or iron" in interaction_items:
    interaction_notes.append("Polyvalent cations reduce integrase-inhibitor absorption: separate BIC/DTG from aluminum or magnesium products by 2 hours before or 6 hours after; calcium/iron may be taken with food with some regimens, but verify the exact product instructions.")
...
summary_lines.append("Baseline labs today: " + "; ".join(baseline_labs) + ".")
