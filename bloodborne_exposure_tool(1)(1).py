...
st.markdown("Wash skin or wounds with soap and water; flush mucous membranes/eyes with water or saline; do not squeeze, milk, scrub, or apply bleach/caustic agents. Report the exposure immediately to occupational health and follow the facility exposure protocol.")
...
hbv_vaccine_schedule = "3-dose series: dose 1 today, dose 2 at 1 month, dose 3 at 6 months"
if hbv_status in ["Unvaccinated or incompletely vaccinated", "Vaccination status unknown"] or (hbv_status == "Complete series; anti-HBs not previously documented" and anti_hbs_available == "Yes" and anti_hbs_value < 10):
    hbv_vaccine_schedule = st.radio(
        "HepB vaccine schedule being used",
        [
            "3-dose series: dose 1 today, dose 2 at 1 month, dose 3 at 6 months",
            "2-dose Heplisav-B: dose 1 today, dose 2 at 1 month",
            "Existing series: continue according to product schedule",
        ],
        key="hbv_vaccine_schedule",
    )
    final_default = add_days(exposure_date, 180) if hbv_vaccine_schedule.startswith("3-dose") else add_days(exposure_date, 30)
    final_hbv_vaccine_date = st.date_input("Actual or planned date of final HepB vaccine dose", value=final_default, min_value=exposure_date, key="final_hbv_vaccine_date")
...
if hbv_status == "Documented responder: complete series and documented anti-HBs ≥10 mIU/mL":
    ...
    if anti_hbs_value >= 10:
        ...
        hbv_actions.append(hbv_decision)
    elif source_hbv in ["Positive", "Unknown / pending"]:
        hbv_decision = "Anti-HBs <10 mIU/mL with HBsAg-positive/unknown source: give HBIG once and initiate revaccination as soon as possible; complete the second vaccine series."
        hbv_actions.append(hbv_decision)
    else:
        hbv_decision = "Anti-HBs <10 mIU/mL with HBsAg-negative source: give one HepB vaccine dose now; repeat anti-HBs 1–2 months after the final vaccine dose."
        hbv_actions.append(hbv_decision)
    ...
    if source_hbv in ["Positive", "Unknown / pending"]:
        hbv_decision = "Give HBIG once and HepB vaccine as soon as possible at separate injection sites; complete the HepB vaccine series."
        hbv_actions.append(hbv_decision)
    else:
...
# Explicit date schedule for the HBV pathway.
hbv_followup = []
hbv_today = evaluation_date
hbv_needs_hbig = source_hbv in ["Positive", "Unknown / pending"] and (hbv_status in ["Unvaccinated or incompletely vaccinated", "Vaccination status unknown", "Documented nonresponder after two complete vaccine series"] or (hbv_status == "Complete series; anti-HBs not previously documented" and anti_hbs_available == "Yes" and anti_hbs_value < 10))
if hbv_status == "Documented responder: complete series and documented anti-HBs ≥10 mIU/mL":
    hbv_followup.append(f"• {fmt(hbv_today)} — No HBV vaccine, HBIG, or HBV follow-up testing required; document documented anti-HBs ≥10 mIU/mL.")
elif hbv_status == "Complete series; anti-HBs not previously documented" and anti_hbs_available == "No":
    hbv_followup.append(f"• {fmt(hbv_today)} — Draw anti-HBs now; do not assume immunity from vaccine history alone.")
    hbv_followup.append("• When anti-HBs results return — If anti-HBs ≥10 mIU/mL, document immunity and stop HBV follow-up; if anti-HBs <10 mIU/mL, follow the susceptible-person pathway below.")
elif hbv_status == "Documented nonresponder after two complete vaccine series":
    if source_hbv in ["Positive", "Unknown / pending"]:
        hbv_followup.append(f"• {fmt(hbv_today)} — Give HBIG dose 1.")
        hbv_followup.append(f"• {later_date(exposure_date, 30)} — Give HBIG dose 2, approximately 1 month after dose 1.")
        hbv_followup.append("• No additional HepB vaccine is routinely indicated after two complete vaccine series.")
    else:
        hbv_followup.append(f"• {fmt(hbv_today)} — No HBV postexposure prophylaxis required because source HBsAg is negative.")
elif final_hbv_vaccine_date:
    dose1 = hbv_today
    if hbv_vaccine_schedule.startswith("2-dose"):
        dose2 = add_days(dose1, 30)
        hbv_followup.extend([
            f"• {fmt(dose1)} — Give HepB vaccine dose 1{'; give HBIG today because source HBsAg is positive/unknown.' if hbv_needs_hbig else '.'}",
            f"• {fmt(dose2)} — Give HepB vaccine dose 2 (final dose).",
        ])
    elif hbv_vaccine_schedule.startswith("3-dose"):
        dose2 = add_days(dose1, 30)
        dose3 = add_days(dose1, 180)
        hbv_followup.extend([
            f"• {fmt(dose1)} — Give HepB vaccine dose 1{'; give HBIG today because source HBsAg is positive/unknown.' if hbv_needs_hbig else '.'}",
            f"• {fmt(dose2)} — Give HepB vaccine dose 2.",
            f"• {fmt(dose3)} — Give HepB vaccine dose 3 (final dose).",
        ])
    else:
        hbv_followup.append(f"• {fmt(dose1)} — Give/continue HepB vaccine dose 1 or the next dose in the existing product schedule{'; give HBIG today because source HBsAg is positive/unknown.' if hbv_needs_hbig else '.'}")
        hbv_followup.append(f"• {fmt(final_hbv_vaccine_date)} — Give the final HepB vaccine dose according to the selected product schedule.")
    anti_hbs_date = add_days(final_hbv_vaccine_date, 60)
    if hbv_needs_hbig:
        anti_hbs_date = max(anti_hbs_date, add_days(hbv_today, 183))
    hbv_followup.append(f"• {fmt(anti_hbs_date)} — Draw anti-HBs 1–2 months after the final vaccine dose; if HBIG was given, do not draw before 6 months after HBIG.")
    if source_hbv in ["Positive", "Unknown / pending"]:
        hbv_followup.append(f"• {later_date(exposure_date, 183)} — Draw HBsAg and total anti-HBc at 6 months after exposure.")
...
if source_hiv_resistance == "Yes": expert_flags.append("Known/suspected source drug resistance")
if pregnancy in ["Pregnant", "Possible / test pending"]: expert_flags.append("Pregnancy or possible pregnancy")
if breastfeeding == "Yes": expert_flags.append("Breastfeeding")
if immunosuppression == "Yes": expert_flags.append("Immunosuppression")
if renal_disease == "Yes": expert_flags.append("Renal disease/abnormal creatinine")
if liver_disease == "Yes": expert_flags.append("Liver disease/abnormal AST or ALT")
if pr_ep == "Yes": expert_flags.append("PrEP or recent injectable ARV exposure")
...
if hiv_pep_indicated:
    regimen_group = st.radio("Select the initial HIV PEP regimen pathway", ["Preferred: BIC/FTC/TAF (Biktarvy) once daily for 28 days", "Preferred: DTG plus TAF or TDF plus FTC or 3TC once daily for 28 days", "Alternative: boosted darunavir plus TAF or TDF plus FTC or 3TC for 28 days", "Need specialist-directed regimen"], key="regimen_group")
...
if hiv_pep_indicated or source_hiv in ["Positive", "Unknown / pending"] or true_exposure:
    ...
    if interim_hiv_needed: hiv_followup.append(f"Interim HIV testing: laboratory HIV Ag/Ab plus diagnostic HIV NAT on {later_date(exposure_date, 42)} (6 weeks; later date selected from the CDC 4–6-week range)")
...
if source_hcv in ["HCV RNA positive", "Anti-HCV positive; RNA unknown", "Unknown / cannot test"]:
    ...
    if immunosuppression == "Yes" or liver_disease == "Yes": hcv_followup.append("Because of immunosuppression or liver disease, consider HCV RNA at the final follow-up even if anti-HCV remains negative.")
...
if hcv_followup: st.markdown("**HCV schedule:**\n" + bullet(hcv_followup))
...
if tetanus in ["No / due", "Unknown"]: precautions.append("Review wound characteristics and administer tetanus vaccination/Tdap or Td according to current wound-management guidance and immunization history.")
st.markdown(bullet(precautions))

# ---------------------------- 9. interactions ----------------------------
st.header("9. HIV PEP medication interaction checker")
interaction_items = st.multiselect("Select any current medication/supplement categories to screen", ["Dofetilide", "Rifampin or rifabutin", "Antacids or supplements containing aluminum, magnesium, calcium, or iron", "Metformin", "Carbamazepine, phenytoin, phenobarbital, or oxcarbazepine", "St. John's wort", "Warfarin or other anticoagulant", "Hormonal contraception or hormone therapy", "Statin or other lipid-lowering medication", "Methadone or buprenorphine", "Other prescription/OTC/herbal medication"], key="interaction_items")
interaction_notes = []
if "Dofetilide" in interaction_items: interaction_notes.append("Dofetilide: contraindicated with dolutegravir and bictegravir; do not select those regimens without specialist direction.")
if "Rifampin or rifabutin" in interaction_items: interaction_notes.append("Rifampin/rifabutin: major interactions with bictegravir, dolutegravir, and boosted regimens; urgent HIV pharmacist/ID consultation is required.")
if "Antacids or supplements containing aluminum, magnesium, calcium, or iron" in interaction_items: interaction_notes.append("Polyvalent cations reduce integrase-inhibitor absorption: separate BIC/DTG from aluminum or magnesium products by 2 hours before or 6 hours after; verify exact product instructions.")
if "Metformin" in interaction_items: interaction_notes.append("Dolutegravir can increase metformin exposure; verify dosing and monitoring with the prescriber/pharmacist.")
if any(x in interaction_items for x in ["Carbamazepine, phenytoin, phenobarbital, or oxcarbazepine", "St. John's wort"]): interaction_notes.append("Enzyme-inducing anticonvulsants and St. John's wort can substantially reduce antiretroviral concentrations; avoid empiric selection without expert consultation.")
if "Hormonal contraception or hormone therapy" in interaction_items: interaction_notes.append("Review the exact hormonal product. Boosted protease-inhibitor regimens have more interaction potential; preferred INSTI-based options generally have fewer clinically important interactions.")
if "Methadone or buprenorphine" in interaction_items: interaction_notes.append("Methadone and buprenorphine can generally be used with preferred PEP regimens, but assess sedation, withdrawal, and adherence clinically.")
if "Other prescription/OTC/herbal medication" in interaction_items or current_meds: interaction_notes.append("Run the complete verified medication list through the Liverpool HIV interaction checker before finalizing the regimen.")
if interaction_notes: st.warning("\n".join(f"- {x}" for x in interaction_notes))
else: st.info("No selected high-risk interaction category. A complete medication reconciliation and formal interaction check are still required.")
...
# The paste-ready section intentionally contains only precautions/education and
# testing, with all tests consolidated under the date they are due.
tests_by_date = {}

def add_test(date_label, test):
    if test:
        tests_by_date.setdefault(date_label, [])
        if test not in tests_by_date[date_label]:
            tests_by_date[date_label].append(test)

today_label = fmt(evaluation_date)
add_test(today_label, "Exposed-person HIV laboratory-based 4th-generation Ag/Ab test (do not delay indicated PEP while awaiting result)")
add_test(today_label, "Exposed-person serum creatinine/eGFR or calculated creatinine clearance")
add_test(today_label, "Exposed-person AST and ALT")
add_test(today_label, "Exposed-person HBV triple panel: HBsAg, anti-HBs, total anti-HBc")
add_test(today_label, "Exposed-person HCV antibody with reflex HCV RNA if positive")
if pregnancy in ["Pregnant", "Possible / test pending"]:
    add_test(today_label, "Pregnancy test if not already confirmed")
if pr_ep == "Yes":
    add_test(today_label, "HIV-1 diagnostic NAT, especially with recent injectable ARV exposure or recent oral PrEP use")
if source_hiv in ["Positive", "Unknown / pending"]:
    add_test(today_label, "Consider HIV-1 diagnostic NAT when clinically indicated, especially with recent ARV exposure or concern for acute HIV infection")

# Source testing that remains pending or is needed.
if source_hiv == "Unknown / pending":
    add_test(today_label, "Source-patient HIV Ag/Ab testing promptly")
if source_hbv == "Unknown / pending":
    add_test(today_label, "Source-patient HBsAg testing promptly")
if source_hcv in ["Anti-HCV positive; RNA unknown", "Unknown / cannot test"]:
    add_test(today_label, "Source-patient HCV RNA testing if possible (or anti-HCV with reflex HCV RNA)")

# HBV testing dates.
if hbv_status == "Complete series; anti-HBs not previously documented" and anti_hbs_available == "No":
    add_test("When available", "Exposed-person anti-HBs; re-evaluate the HBV pathway when the result returns")
elif final_hbv_vaccine_date:
    anti_hbs_date = add_days(final_hbv_vaccine_date, 60)
    if hbv_needs_hbig:
        anti_hbs_date = max(anti_hbs_date, add_days(hbv_today, 183))
    add_test(fmt(anti_hbs_date), "Exposed-person anti-HBs 1–2 months after the final HepB vaccine dose; if HBIG was given, do not draw before 6 months after HBIG")
    if source_hbv in ["Positive", "Unknown / pending"]:
        add_test(fmt(add_days(exposure_date, 183)), "Exposed-person HBsAg and total anti-HBc at 6 months after exposure")
elif hbv_status == "Complete series; anti-HBs not previously documented" and anti_hbs_available == "Yes" and anti_hbs_value < 10 and source_hbv in ["Positive", "Unknown / pending"]:
    add_test(fmt(add_days(exposure_date, 183)), "Exposed-person HBsAg and total anti-HBc at 6 months after exposure")

# HIV testing dates.
if hiv_pep_indicated or source_hiv in ["Positive", "Unknown / pending"] or true_exposure:
    add_test(today_label, "Exposed-person HIV laboratory-based 4th-generation Ag/Ab test; HIV-1 diagnostic NAT when indicated")
    if hiv_pep_indicated and (hours_since > 24 or pr_ep == "Yes" or source_hiv_resistance == "Yes" or immunosuppression == "Yes"):
        add_test(fmt(add_days(exposure_date, 42)), "Interim HIV laboratory Ag/Ab plus diagnostic HIV-1 NAT at 6 weeks")
    add_test(fmt(add_days(exposure_date, 84)), "Final HIV laboratory Ag/Ab plus diagnostic HIV-1 NAT at 12 weeks")

# HCV testing dates.
if hcv_followup:
    add_test(today_label, "Exposed-person anti-HCV with reflex HCV RNA if positive (preferably within 48 hours)")
    add_test(fmt(add_days(exposure_date, 42)), "Exposed-person HCV RNA NAT at 6 weeks")
    add_test(fmt(add_days(exposure_date, 183)), "Exposed-person HCV anti-HCV with reflex HCV RNA if positive at 6 months")
    if immunosuppression == "Yes" or liver_disease == "Yes":
        add_test(fmt(add_days(exposure_date, 183)), "Consider HCV RNA at final follow-up even if anti-HCV remains negative")

assessment_plan = [
    "Assessment/Plan — precautions/education:",
    "• Wash skin or wounds with soap and water; flush mucous membranes/eyes with water or saline; do not squeeze, milk, scrub, or apply bleach or other caustic agents.",
    "• Report the exposure immediately to occupational health and follow the facility exposure protocol.",
]
assessment_plan.extend([f"• {item}" for item in precautions])
assessment_plan.append("")
assessment_plan.append("Testing needed by date:")
for date_label, tests in tests_by_date.items():
    assessment_plan.append(f"{date_label}:")
    assessment_plan.extend([f"• {test}" for test in tests])

summary_text = "\n".join(assessment_plan)
st.text_area("Assessment/plan text", value=summary_text, height=700, key="summary_text")
