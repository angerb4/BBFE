...
summary_lines = []
summary_lines.append(f"Assessment: Occupational bloodborne pathogen exposure on {fmt(exposure_date)} ({timing_label} at evaluation). Exposure: {exposure_type}; source material: {fluid}.")
summary_lines.append("Plan — immediate actions:")
summary_lines.append("• Immediately: Wash skin/wounds with soap and water; irrigate eyes/mucous membranes with water or saline; do not squeeze, milk, scrub, or apply caustic agents.")
summary_lines.append("• Immediately: Report to occupational health and follow the facility exposure protocol.")
summary_lines.append(f"Plan — baseline testing on {fmt(evaluation_date)}:")
summary_lines.extend([f"• {x}" for x in baseline_labs])
summary_lines.append("Plan — Hepatitis B:")
summary_lines.extend(hbv_followup or [f"• {fmt(evaluation_date)} — Complete the HBV decision pathway and document source HBsAg and exposed-person immune status."])
summary_lines.append("Plan — HIV:")
summary_lines.append(f"• {fmt(evaluation_date)} — {hiv_reason}")
if hiv_pep_indicated:
    summary_lines.append(f"• {fmt(evaluation_date)} — Start HIV PEP immediately: {hiv_regimen}")
    summary_lines.append(f"• {later_date(exposure_date, 1)} — Clinical/medication access check within 24 hours.")
    summary_lines.append(f"• {later_date(exposure_date, 3)} — Clinical reassessment by 72 hours after exposure.")
    if interim_hiv_needed: summary_lines.append(f"• {later_date(exposure_date, 42)} — Interim HIV Ag/Ab plus HIV-1 diagnostic NAT (6 weeks).")
    summary_lines.append(f"• {later_date(exposure_date, 84)} — Final HIV Ag/Ab plus HIV-1 diagnostic NAT (12 weeks).")
elif hiv_followup:
    summary_lines.append(f"• {later_date(exposure_date, 84)} — Final HIV testing if follow-up remains indicated.")
summary_lines.append("Plan — Hepatitis C:")
if hcv_followup: summary_lines.extend([f"• {x}" for x in hcv_followup])
else: summary_lines.append(f"• {fmt(evaluation_date)} — {hcv_decision}")
summary_lines.append("Plan — precautions/counseling:")
summary_lines.extend([f"• {x}" for x in precautions])
if tetanus in ["No / due", "Unknown"]: summary_lines.append(f"• {fmt(evaluation_date)} — Review immunization history and wound category; administer Tdap/Td if indicated.")
else: summary_lines.append(f"• {fmt(evaluation_date)} — Verify tetanus immunization documentation and wound-specific indication.")
summary_lines.append("• Any date — Seek immediate medical care for fever, rash, lymphadenopathy, severe fatigue, sore throat, jaundice, dark urine, abdominal pain, nausea/vomiting, unusual bleeding, medication toxicity, or allergic reaction.")
if expert_flags:
    summary_lines.append("Plan — expert consultation:")
    summary_lines.extend([f"• As soon as possible — Consult HIV/ID/occupational-health experts for: {x}." for x in expert_flags])
